...
public class CompanyFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		System.out.println("Company filter: " + req.getMethod());

		String token = req.getHeader("token");
		if (token != null) {
			Session session = sessionContext.getSession(token);
			if ((session != null) && (session.getAttribute("clientType").equals(ClientType.COMPANY))) {
				System.out.println("Company session good - forward the request");
