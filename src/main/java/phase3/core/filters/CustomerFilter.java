...
public class CustomerFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		System.out.println("Customer filter: " + req.getMethod());
		
		String token = req.getHeader("token");
		if(token != null) {
			Session session = sessionContext.getSession(token);
			if((session != null)&&(session.getAttribute("clientType").equals(ClientType.CUSTOMER))) {
				System.out.println("customer session good - forward the request");
